...
* 制約: サーバーの buildx が 0.12.1 で Compose v5.5.1 の `compose build` 要件 (0.17.0) に満たないため、
  イメージをビルドせず素の `python:3.12-slim` にスクリプトをバインドマウントしている。
  標準ライブラリ以外に依存したくなったら、先に buildx を上げる必要がある
