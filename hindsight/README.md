...
scanner/   識別子の破損を定期検出する常駐サービス (下記の既知の罠への対症療法)
```
...
| 識別子の破損を確認 | サーバーで `cat /data/hindsight/scanner/report.json`（[`scanner/`](scanner/README.md)。`finding_count: 0` なら破損なし） |
...
- **出力言語を固定すると識別子保護が巻き添えで外れる。** `HINDSIGHT_API_LLM_OUTPUT_LANGUAGE` を設定すると、`engine/consolidation/prompts.py` は言語ルールを二者択一で組み立てているため、未設定のときだけ入る `_DEFAULT_LANGUAGE_RULE`（"Proper nouns, identifiers, and units stay verbatim." を含む）がブロックごと落ち、代わりに `prompt_utils.output_language_directive` の「エンティティ名を含め全て翻訳しろ」だけが残る。結果 identifier が日本語化されて潰れる (`hindsight-mcp-api-key` → `hindsightmcpapikey`、`t4g.medium` → `t4gmedium`)。retain 側 (`engine/retain/fact_extraction.py`) はさらに悪く、テンプレート本体の「翻訳は STRICTLY FORBIDDEN」(`:822`) と末尾の翻訳ディレクティブ (`:1278`) が**同時に載る** — consolidation にある排他制御が無い。このため 2026-09-23 にこの設定をやめた ([ADR 0019](../docs/adr/0019-output-language-via-retain-mission.md))。
- **未設定だと retain が投入テキストを別言語に翻訳することがある。** 地の文の日本語が少なく、コード片や識別子が濃い入力では、英語や中国語の fact が保存される (`dry-run-extract` で実測: 8 回中 2 回しか日本語を保たなかった)。バンクの `retain_mission` に「出力は地の文の言語で書く。コード片・ファイルパス・識別子・英語の引用が多く含まれていても、それらは言語判定の材料にしない。」を置くと 5/5 で日本語になった。consolidation 側は同じ検証ができない (`dry-run-extract` は retain しか通らない) ので、[`scanner/`](scanner/README.md) の `finding_count` で事後に見る。
...
- **`reflect` の wall timeout は既定 300 秒しかなく、正常系を切る。** mental model / ナレッジページの本文生成は `reflect_async` を通るため、この制限がそのまま効く（公式ドキュメント configuration の 「Internal recall (used by mental model refresh)」節）。タグで絞れないページの生成は実測 276 秒かかり、記憶が増えれば確実に超える。`compose/docker-compose.yml` で `HINDSIGHT_API_REFLECT_WALL_TIMEOUT=900` に上げている。retain 側の `HINDSIGHT_API_RETAIN_WALL_TIMEOUT` は既定 3600 秒で、`worker/poller.py` のコメントも「本当に詰まったときだけ発火させる」と書いている。reflect だけ 5 分という非対称に注意。なお探索は `HINDSIGHT_API_REFLECT_MAX_ITERATIONS=10` で打ち切られるので、時間を与えても無限には走らない。
