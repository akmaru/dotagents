// /web-research — 同梱 /deep-research（Claude Code 2.1.280 の実行時に永続化されたスクリプト）の写し。
// 同梱版は agent() に model を渡さないためセッションのモデルで 100 体前後が回る。段階ごとにモデルを
// 指定できるようにするためだけに複製している。それ以外の変更はしない（差分が追えるように）。
// /deliberate の kind: 'web' の小問はこれを workflow('web-research', …) で呼ぶ（docs/adr/0020）。
export const meta = {
  name: 'web-research',
  description: 'Deep research harness (model-pinned copy of /deep-research) — fan-out web searches, fetch sources, adversarially verify claims, synthesize a cited report.',
  whenToUse: 'When the user wants a deep, multi-source, fact-checked research report on any topic, with the model pinned per stage (args.models). BEFORE invoking, check if the question is specific enough to research directly — if underspecified (e.g., "what car to buy" without budget/use-case/region), ask 2-3 clarifying questions to narrow scope. Then, ALWAYS ask the user with AskUserQuestion, in one question: whether to run it at all (one run spawns ~100 agents, ~10M tokens, hours), and the model per stage — scope / search / fetch / verify / synthesize, each defaulting to opus (offer opus / sonnet / haiku). Pass only the stages changed from the default as args.models, and the refined question as args: {question, models}. Skip the model question only after the user said to use defaults.',
...
// Question is passed via Workflow({name: 'web-research', args: '<question>'}) or args: {question, models}.

// 段階ごとのモデル。既定はすべて opus（2026-09-24 のユーザー決定）。args.models で部分上書き
const MODELS = {
  scope: 'opus',
  search: 'opus',
  fetch: 'opus',
  verify: 'opus',
  synthesize: 'opus',
  ...(args && typeof args === 'object' && args.models ? args.models : {}),
}
...
if (!QUESTION) {
  return { error: "No research question provided. Pass it as args: Workflow({name: 'web-research', args: '<question>'}) or args: {question, models}." }
}
const scope = await agent(
  ...
  { label: "scope", model: MODELS.scope, schema: SCOPE_SCHEMA }
)
if (!scope) {
...
const searchResults = await pipeline(
  ...
  angle => agent(SEARCH_PROMPT(angle), {
    label: "search:" + angle.label, phase: "Search", model: MODELS.search, schema: SEARCH_SCHEMA
  }).then(r => {
    if (!r) return null
...
const report = await agent(
  ...
  { label: "synthesize", model: MODELS.synthesize, schema: REPORT_SCHEMA }
)
